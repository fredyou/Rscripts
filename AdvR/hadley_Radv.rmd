...
#6Internal generics don鈥檛 dispatch on the implicit class of base types. 
...
# All S4 related code is stored in the methods package. This package is always available when you鈥檙e running R interactively, but may not be available when running R in batch mode. For this reason, it鈥檚 a good idea to include an explicit library(methods) whenever you鈥檙e using S4.
...
###Exercise2 What happens if you define a new S4 class that doesn鈥檛 鈥渃ontain鈥? an existing class? (Hint: read about virtual classes in ?Classes.)
...
### composite integration. I鈥檒l implement it using two new functions:
...
#mapply has the MoreArgs argument that takes a list of extra arguments that will be supplied, as is, to each call. This breaks R鈥檚 usual lazy evaluation semantics, and is inconsistent with other functions.
...
outer(1:3,1:10,"*")
#outer(琛?, 鍒?,"*"), 琛屽厓绱?1涓庢墍鏈夊垪杩涜"*" 鐒跺悗琛?2....
...
###Exercise2: There鈥檚 no equivalent to split() + vapply(). Should there be? When would it be useful? Implement one yourself.
pulse <- round(rnorm(22, 70, 10 / 3)) + rep(c(0, 5), c(10, 12))
group <- rep(c("A", "B"), c(10, 12))
data=split(pulse, group)
...
###Exercise1: Why isn鈥檛 is.na() a predicate function? What base R function is closest to being a predicate version of is.na()?
...
#First, we create a function factory that, given a dataset, returns a function that computes the negative log likelihood (NLL) for parameter lambda. In R, it鈥檚 common to work with the negative since optimise() defaults to finding the minimum.
...
#If you need to modify part of an existing data frame, it鈥檚 often better to use a for loop. 
#2. Recursive relationships
#It鈥檚 hard to convert a for loop into a functional when the relationship between elements is not independent, or is defined recursively.
...
#Reduce(),  If we give Reduce() a length one vector, it doesn鈥檛 have anything to reduce, so it just returns the input. If we give it an input of length zero, it always returns NULL. 
...
###??Exercise3: How does paste() fit into this structure? What is the scalar binary function that underlies paste()? What are the sep and collapse arguments to paste() equivalent to? Are there any paste variants that don鈥檛 have existing R implementations?
...
##################################
#it can be hard to see what鈥檚 going on inside of functionals
...
} #鑰冭檻涓変釜閫昏緫锛氱涓�娆¤繍琛岋紝寤惰瓒呮椂鍚庣殑杩愯鍜屼笉瓒呮椂鐨勮繍琛?
...
} #BETTER!!!鑰冭檻涓や釜閫昏緫 婊¤冻delay鍜屼笉婊¤冻delay:
  #婊¤冻delay: 瑕佸欢杩熺殑鏃堕棿灏忎簬褰撳墠鏃堕棿锛堢涓�娆¤繍琛屽拰瓒呮椂杩愯锛?
  #涓嶆弧瓒砫elay: 瑕佸欢杩熷埌鐨勬椂闂村ぇ浜庡綋鍓嶆椂闂?,绋嬪簭鎷栧欢鍒板欢杩熸椂闂?

delay_by3 <- function(delay, f)
{
  executime <- 0
  #濡傛灉鎹㈡垚<-Sys.time(),鎵ц鏃堕棿灏变笉瀵逛簡,鍥犱负绗竴娆℃墽琛屽苟涓嶆槸Sys.time
  #寰楀埌缁忛獙:鏁板�?,鏃堕棿鐨勫垵濮嬪�兼渶濂芥槸0
  function(...)
  {
    if(Sys.time()-executime < delay) {
      print(delay-(Sys.time()-executime))
      Sys.sleep(delay-(Sys.time()-executime))
    }#璺濈涓婃call鏃堕棿鍦╠elay鍐?,鍒檇elay
    res <- f(...)
    executime <<- Sys.time()
    res
  }  
}#浣嗘槸娉ㄦ剰Sys.time()-executime < delay ->Sys.time()< delay+executime
 #鍦╡xecutime=0鐨勬椂鍊?,濡傛灉delay 寮傚父澶?,澶ц繃鐜板湪鏃堕棿鐨勮瘽,浼氬彉鎴怲rue
 #浣嗗鏋滃彉鎹㈠紡瀛愪负Sys.time()-delay<executime,鍒欏彲璁惧垵濮媏xecutime涓?-inf,鍒欏垵濮嬪繀false
 #Sys.time()-delay< -(Inf)
 #鍦╠elay2 func涓璬elay_until - Sys.time() > 0 ,鍦ㄧ涓�娆¤繍琛屾椂姘镐笉涓簍rue(Sys.time-Sys.time==0!>0)
...
#####Note: if 涓殑condition 涓嶇瓑寮忓乏鍙虫崲浣嶅彲鏈夊ぇ涓嶅悓.
#濡備綍鎺ㄥdelay_until - Sys.time() > 0杩欎釜鏉′欢?
########## 鑰冭檻涓や釜鍔ㄦ�佸彉鍖栫殑鏃堕棿鐐?(鏁版嵁)鏃?,鏈�濂界敾涓ゆ潯绾?
#   establish    call      call2
# -----*-----------*--*----*---
#                  *----*
#       {         }{delay}
#{}{delay}鐨勯暱搴︽渶闀?,绗竴娆all鏃秂stablish鐨勬椂闂寸煭浜庢垨绛変簬call鐨勬椂闂?:time<-Sys.time;funciton(){if(time<=sys.time).... 涔熷嵆time <=sys.time
#绗簩娆all鐨勬椂鍊檆all2鐨剆ystem.time>=call1time+delay,涔熷嵆call1time+delay<=sys.time
#杩欐牱鐨勮瘽establish time灏卞簲璇ユ寜鐓alltime+delay鏉ュ彇鍚?,绗竴娆stablish time涓簊ys.time of FO ,call涔嬪悗涓簊ys.time of closure+delay
...
#It is a function that has certain arguments 鈥渇illed in鈥? already. This is called 鈥減artial function application鈥?, and is implemented by pryr::partial().
...
###Exercise2: Read the source code for plyr::colwise(). How does the code work? What are colwise()鈥檚 three main tasks? How could you make colwise() simpler by implementing each task as a function operator? (Hint: think about partial().)
...
### In this style of programming, you don鈥檛 explicitly refer to variables. Instead, you focus on the high-level composition of functions rather than the low-level flow of data. The focus is on what鈥檚 being done, not on objects it鈥檚 being done to. Since we鈥檙e using only functions and not parameters, we use verbs and not nouns. This style is common in Haskell
###compose() is particularly useful in conjunction with partial(), because partial() allows you to supply additional arguments to the functions being composed. One nice side effect of this style of programming is that it keeps a function鈥檚 arguments near its name.
...
###Exercise3:Implement the xor() binary operator. Implement it using the existing xor() function. Implement it as a combination of and() and or(). What are the advantages and disadvantages of each approach? Also think about what you鈥檒l call the resulting function to avoid a clash with the existing xor() function, and how you might change the names of and(), not(), and or() to keep them consistent.
and <- function(f1, f2) {
  force(f1); force(f2)
  function(...) {
    f1(...) && f2(...)
...
###Exercise4:f(), defined above, just calls substitute(). Why can鈥檛 we use it to define g()? In other words, what will the following code return? First make a prediction. Then run the code and think about the results.
...
###quote:  captures an unevaluated expression like substitute(), but doesn鈥檛 do any of the advanced transformations that can make substitute() confusing
###quote() and eval() are opposites. In the example below, each eval() peels off one layer of quote()鈥檚
...
###这里可以将subset2改为接收substitute(condition)为参数，而不是在subset2里再substitute
sample_df <- data.frame(a = 1:5, b = 5:1, c = c(5, 3, 1, 4, 1))
...
###这里capCon里的substitue的scope是capCon而不是调用capCon的函数,故而subset2_q eval parent.frame时并不在capCon内,而应该是capCon的parent.frame()
###因此,应该用capCon<-function(condiciton){substitute(condition, parent.frame())}:
capCon<-function(condition){
  substitute(condition,parent.frame())
}
subscramble(sample_df, a >= 4)
###这里subset2_q和capCon都用parent.frame,因此,连用时scope一致了,这部分可能换成env参数可调为最合适
...
#This is cuz   cols <- as.list(substitute(list(a<-a-1))[-1]) 会变成没有名字的值为a<-a-1的list
#             而as.list(substitute(list(a=-a-1))[-1]) 会变成名字为a的值为a-1的list

# as.list(substitute(list()))会识别里面的等号，将=前面的名字存起来
mutate=function (.data, ...) 
{
    stopifnot(is.data.frame(.data) || is.list(.data) || is.environment(.data))
