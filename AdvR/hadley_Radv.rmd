...
delay_by2 <- function(delay, f)
{
  ...
  #不满足delay: 要延迟到的时间大于当前时�?,程序拖延到延迟时�?
...
delay_by3 <- function(delay, f)
{
  ...
  function(...)
  {
    ...
    }#距离上次call时间在delay�?,则delay
    res <- f(...)
    executime <<- Sys.time()
    res
  }  
}#但是注意Sys.time()-executime < delay ->Sys.time()< delay+executime
 #在executime=0的时�?,如果delay 异常�?,大过现在时间的话,会变成True
 #但如果变换式子为Sys.time()-delay<executime,则可设初始executime�?-inf,则初始必false
 #Sys.time()-delay< -(Inf)
...
########## 考虑两个动态变化的时间�?(数据)�?,最好画两条�?
#   establish    call      call2
# -----*-----------*--*----*---
#                  *----*
#       {         }{delay}
#{}{delay}的长度最�?,第一次call时establish的时间短于或等于call的时�?:time<-Sys.time;funciton(){if(time<=sys.time).... 也即time <=sys.time
#第二次call的时候call2的system.time>=call1time+delay,也即call1time+delay<=sys.time
#这样的话establish time就应该按照calltime+delay来取�?,第一次establish time为sys.time of FO ,call之后为sys.time of closure+delay
...
###Exercise4:f(), defined above, just calls substitute(). Why can’t we use it to define g()? In other words, what will the following code return? First make a prediction. Then run the code and think about the results.
...
###quote:  captures an unevaluated expression like substitute(), but doesn’t do any of the advanced transformations that can make substitute() confusing
...
###������Խ�subset2��Ϊ����substitute(condition)Ϊ��������������subset2����substitute
sample_df <- data.frame(a = 1:5, b = 5:1, c = c(5, 3, 1, 4, 1))
...
#This is cuz   cols <- as.list(substitute(list(a<-a-1))[-1]) ����û�����ֵ�ֵΪa<-a-1��list
#             ��as.list(substitute(list(a=-a-1))[-1]) ��������Ϊa��ֵΪa-1��list

# as.list(substitute(list()))��ʶ������ĵȺţ���=ǰ������ִ�����
mutate=function (.data, ...) 
{
    stopifnot(is.data.frame(.data) || is.list(.data) || is.environment(.data))
